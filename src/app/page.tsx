  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden bg-[#050505] text-zinc-100">
        ...
        <motion.div className="mx-auto max-w-3xl text-center" variants={heroContainer} initial={animate ? "hidden" : false} animate="show">
          ...
          <motion.h1 variants={heroItem} className="mt-6 text-[2.1rem] font-semibold leading-[1.12] tracking-tight sm:text-5xl md:text-[3.25rem]">
            ...
            <span className="bg-gradient-to-r from-omniv-gold via-amber-200 to-omniv-gold bg-clip-text text-transparent">
              Meet Ziki — your AI career OS.
            </span>
          </motion.h1>
          <motion.p variants={heroItem} className="mx-auto mt-5 max-w-xl text-[15px] leading-relaxed text-zinc-400 md:text-base">
            Stop drowning in daily music chaos. Get a clear weekly plan: own your fans, open ticketed rooms, tip links, and ranked next moves — so you spend more time on music and less time guessing.
          </motion.p>
          ...
          <div id="blueprint" className="relative mx-auto mt-14 max-w-2xl overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-white/[0.07] to-transparent p-1 shadow-2xl shadow-black/50">
            <div className="rounded-[1.35rem] bg-[#0a0a0a] p-5 md:p-7">
              <div className="flex items-center justify-between gap-2">
                <p className="font-data text-[10px] uppercase tracking-[0.2em] text-omniv-gold">This week’s blueprint</p>
              ...
              <ul className="mt-4 space-y-2.5 text-left text-[13px] text-zinc-300">
                <li className="rounded-xl border border-white/8 bg-white/[0.03] px-3.5 py-3">
                  <span className="text-omniv-gold">1.</span> Share fan gate — collect city + “would attend”
                </li>
                <li className="rounded-xl border border-white/8 bg-white/[0.03] px-3.5 py-3">
                  <span className="text-omniv-gold">2.</span> Lagos line: ~80 fans, 15% intent → try $5 ticket, ~50-cap room
                </li>
                <li className="rounded-xl border border-white/8 bg-white/[0.03] px-3.5 py-3">
                  <span className="text-omniv-gold">3.</span> Open room + tip link in bio before Friday drop
                </li>
              </ul>
              <p className="mt-4 text-left text-[12px] text-zinc-600">
                Example of how Omniv ranks moves from your data — not random tips.
              </p>
            </div>
          </div>
        </FadeUp>
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 py-14 md:px-8">
        <FadeUp>
          ...
          <h2 className="mx-auto mt-2 max-w-2xl text-center text-2xl font-semibold tracking-tight md:text-3xl">
            You can keep fighting the algorithm alone — or run a real plan.
          </h2>
        </FadeUp>
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          <FadeUp>
            <div className="h-full rounded-2xl border border-white/10 bg-white/[0.02] p-6">
              <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">The messy reality</p>
              <ul className="mt-4 space-y-3 text-[14px] leading-relaxed text-zinc-400">
                <li>Most of the day goes to hooks, captions, and hoping.</li>
                <li>Playlists ghost you. Strategy is a guess.</li>
                <li>Streams look big. Money stays small.</li>
                <li>You still don’t know who would show up to a room.</li>
              </ul>
            </div>
          </FadeUp>
          <FadeUp delay={0.08}>
            <div className="h-full rounded-2xl border border-omniv-gold/30 bg-omniv-gold/5 p-6">
              <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-omniv-gold">The Omniv reality</p>
              <ul className="mt-4 space-y-3 text-[14px] leading-relaxed text-zinc-200">
                <li>More time on music. Ziki ranks the next moves.</li>
                <li>Fan gates turn listeners into a list you own.</li>
                <li>Rooms + tips = cash paths you control.</li>
                <li>Demand lines: city, intent, ticket idea, room size.</li>
              </ul>
            </div>
          </FadeUp>
        </div>
      ...
      <section id="ecosystem" className="relative z-10 mx-auto max-w-6xl px-5 py-14 md:px-8">
        <FadeUp>
          <p className="text-center text-[10px] font-semibold uppercase tracking-[0.18em] text-omniv-gold">Inside your music career OS</p>
          <h2 className="mx-auto mt-2 max-w-xl text-center text-2xl font-semibold tracking-tight md:text-3xl">Three engines. Plain language.</h2>
        </FadeUp>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {[
            { icon: ListChecks, t: "Weekly moves", d: "No more “what now?” Ziki and Moves give a short list of high-impact tasks — open a room, post the tip link, draft one pitch." },
            { icon: Users, t: "Fan gates (own your people)", d: "Turn casual listeners into emails and cities on a list you control. Mark “would attend” so you know who might show up." },
            { icon: CalendarRange, t: "Release + content plan", d: "Score a drop window. Line up content before the date. Connect catalogue and money tools so the release can actually pay." },
          ].map((f, i) => (
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {[
            { icon: Radio, t: "Agent (outside signals)", d: "Market news for labels, sync, and playlists — filtered for music, with a link to the source." },
            { icon: Wallet, t: "Money", d: "Tip links and ticketed rooms. Fans pay you. Split toward your bank when payout is set up." },
          ].map((f, i) => (
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 py-8 md:px-8">
        <FadeUp>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-4 text-center text-[13px] text-zinc-400 md:text-[14px]">
            Built for independents who want <span className="text-omniv-gold">owned fans</span>,{" "}
            <span className="text-omniv-gold">clear weekly moves</span>, and <span className="text-omniv-gold">cash paths</span> that aren’t only streams — rooms, tips, and ranked actions.
          </div>
        </FadeUp>
      </section>
      ...
      <section id="how" className="relative z-10 mx-auto max-w-6xl px-5 py-14 md:px-8">
        <FadeUp>
          ...
          <h2 className="mx-auto mt-2 max-w-xl text-center text-2xl font-semibold tracking-tight md:text-3xl">Three steps. No thick manual.</h2>
        </FadeUp>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {[
            { n: "1", t: "Connect your world", d: "Sign up. Add your goal and style. Link catalogue or socials when you’re ready.", icon: Music2 },
            { n: "2", t: "Meet Ziki", d: "Scan your artist brain. See demand ideas and outside signals that fit your path.", icon: Sparkles },
            { n: "3", t: "Execute one move", d: "Share the fan gate, open a room, or drop a tip link. Come back for the next ranked step.", icon: ListChecks },
          ].map((s, i) => (
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 py-16 text-center md:px-8">
        <FadeUp>
          <h2 className="text-2xl font-semibold tracking-tight md:text-3xl">The industry moves fast. Don’t wing it alone.</h2>
          <p className="mx-auto mt-3 max-w-md text-[14px] text-zinc-500">
            Treat your art like a serious career. Free to start — no credit card required.
          </p>
          <Link href="/signup" className="mt-8 inline-block">
            <Button className="h-12 rounded-full bg-omniv-gold px-10 text-[15px] font-semibold text-black hover:bg-omniv-gold/90">
              Initialize my career OS
              <ArrowRight className="ml-1.5 h-4 w-4" />
            </Button>
          </Link>
        </FadeUp>
          ...
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
            <div className="lg:col-span-2">
              ...
              <p className="mt-3 max-w-xs text-[13px] leading-relaxed text-zinc-500">
                AI career OS for independent artists. Ranked moves, fan gates, rooms, and tips — so you know what to do next.
              </p>
