...
export default function AnalyticsPage() {
  ...
  const [hasSpotifyLinks, setHasSpotifyLinks] = useState(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const spotifyN = releases.filter((r) => {
          const any = r as unknown as Record<string, unknown>;
          const u =
            any.spotifyUrl ||
            any.spotify_url ||
            (any.links as { spotify?: string } | undefined)?.spotify;
          return Boolean(u && String(u).length > 8);
        }).length;
        if (!cancelled) setHasSpotifyLinks(spotifyN > 0);
  ...
  return (
    <AppShell>
      ...
      <div className="mb-4">
        <DspMetricsRefresh hasSpotifyLinks={hasSpotifyLinks} />
      </div>
