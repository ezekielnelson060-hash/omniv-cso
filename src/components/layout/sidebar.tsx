  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && profile?.display_name) setName(profile.display_name);
      } catch {
        /* soft */
      }
      ...
      try {
        ...
        if (!cancelled && data.user?.email && !name) {
          setName(data.user.email.split("@")[0] || null);
        }
      } catch {
        /* soft */
      }
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2.5 px-3 py-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[14px] font-semibold tracking-tight">Omniv</p>
          {name && (
