...
export function LiveRoom() {
  ...
  const presenceKey = useRef(
    `u-${typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID().slice(0, 12) : `${Date.now()}`}`
  );
  ...
  useEffect(() => {
    ...
    function applyPresence(ch: NonNullable<typeof channelRef.current>) {
      try {
        ...
        for (const key of Object.keys(state)) {
          ...
          const meta = metas[0];
          names.push(
            (meta?.name || "").trim() || (meta?.host ? "Host" : "Listener")
          );
        }
        setPresenceNames(names.slice(0, 24));
        setListeners(Math.max(1, names.length));
      } catch {
        /* soft */
      }
    }
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (row.kind === "tip") {
              setTipsFlash((f) =>
                [`${row.display_name}: ${row.body}`, ...f].slice(0, 5)
              );
            }
          }
        )
        .on(
          ...
          (payload) => {
            ...
            if (row.now_playing_url !== undefined)
              setPlayUrl(row.now_playing_url || "");
            if (row.now_playing_title !== undefined)
              setPlayTitle(row.now_playing_title || "");
            if (row.industry_guest_name !== undefined)
              setGuestName(row.industry_guest_name || "");
            if (row.industry_guest_role !== undefined)
              setGuestRole(row.industry_guest_role || "");
            if (row.industry_guest_active !== undefined)
              setGuestOn(Boolean(row.industry_guest_active));
          }
        )
        .on("presence", { event: "sync" }, () => {
          if (channelRef.current) applyPresence(channelRef.current);
        })
        ...
        .subscribe(async (status) => {
          if (status === "SUBSCRIBED") {
            ...
            const display =
              ...
              )?.value?.trim() || (isHost ? "Host" : "Listener");
            try {
              await channel.track({
            ...
            try {
              ...
              track("room_presence", { gathering_id: id, host: isHost });
            } catch {
              /* soft */
            }
          } else if (
            ...
            setRealtimeOk(false);
            if (!poll) {
              poll = window.setInterval(() => void loadMessages(), 8000);
            }
          }
        });
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] text-omniv-text-muted">
                ...
                {listeners} in room
                {realtimeOk ? (
                  <span className="ml-1 text-[9px] text-emerald-400/90">· live</span>
                ) : (
                  <span className="ml-1 text-[9px] text-omniv-text-muted">· syncing</span>
                )}
              </span>
            </div>
          ...
          {presenceNames.length > 0 && (
            <p className="truncate text-[10px] text-omniv-text-muted">
              Here now: {presenceNames.slice(0, 8).join(" · ")}
              {presenceNames.length > 8 ? ` · +${presenceNames.length - 8}` : ""}
            </p>
          )}
