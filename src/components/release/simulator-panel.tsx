...
import {
  ...
  spotifyEditorialCuration,
  appleMusicAlgorithmicRadio,
  buildVerdictTree,
  type PlaylistSignal,
  type TikTokSignal,
  type EditorialSignal,
  type AppleRadioSignal,
  type DecisionNode,
} from "@/lib/strategy/distribution-signals";
import { DecisionTree } from "@/components/release/decision-tree";
import { DistributionBriefings } from "@/components/release/distribution-briefings";
...
export function SimulatorPanel() {
  ...
  const [editorial, setEditorial] = useState<EditorialSignal | null>(null);
  const [appleRadio, setAppleRadio] = useState<AppleRadioSignal | null>(null);
  ...
  async function run() {
    ...
    try {
      ...
      setEditorial(spotifyEditorialCuration(input, sim.primary, brain));
      setAppleRadio(appleMusicAlgorithmicRadio(input, sim.primary));
    ...
    <div className="space-y-5">
      <Card className="p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              placeholder="Who it is for and why it belongs this week"
            />
          </div>
          <Input
        ...
        >
          ...
          {busy ? "Running simulation…" : "Run the simulation"}
        </Button>
      ...
      {result && (
        <div className="space-y-4 animate-fade-in-up">
          ...
          <DistributionBriefings
            playlist={playlist}
            editorial={editorial}
            appleRadio={appleRadio}
            tiktok={tiktok}
          />
          ...
          <Card className="p-5">
            <p className="text-sm font-medium">Pre-flight — do not skip</p>
            <ul className="mt-3 space-y-2">
